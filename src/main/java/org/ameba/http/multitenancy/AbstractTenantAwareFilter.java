 ...
 * <p>
 * The tenant information is expected to be present as http header attribute with name {@value Constants#HEADER_VALUE_X_TENANT}. The
 * behavior of the attribute validation can be configured via two properties:
 * <ul>
 *     <li>{@value Constants#PARAM_MULTI_TENANCY_ENABLED}: If {@literal true}, the filter expects the tenant attribute set in the current
 *          request and continues with validation and processing the tenant information.
 *     </li>
 *     <li>{@value Constants#PARAM_MULTI_TENANCY_THROW_IF_NOT_PRESENT}: If the tenant attribute is not present in the current request and
 *          this property is set to {@literal true}, the filter will throw an exception. If set to {@literal false} the filter ignores the
 *          missing attribute and continues in the {@code filterChain} without calling the subclasses
 *          {@link #doBefore(HttpServletRequest, HttpServletResponse, FilterChain, String)} method.
 *     </li>
 * </ul>
 *
...
public abstract class AbstractTenantAwareFilter extends OncePerRequestFilter {
     ...
     */
    @Override
    protected final void doFilterInternal(HttpServletRequest r, HttpServletResponse response, FilterChain filterChain)
            throws ServletException, IOException {
        var fromSC = (String) r.getServletContext().getAttribute(Constants.PARAM_MULTI_TENANCY_ENABLED);
        var multiTenancyEnabled = Boolean.parseBoolean(
                fromSC == null
                        ? getFilterConfig().getInitParameter(Constants.PARAM_MULTI_TENANCY_ENABLED)
                        : fromSC
        );
        String tenant = null;
        if (multiTenancyEnabled && !"OPTIONS".equalsIgnoreCase(r.getMethod())) {
            ...
            if (null == tenant || tenant.isEmpty()) {
                var throwParam = (String) r.getServletContext().getAttribute(Constants.PARAM_MULTI_TENANCY_THROW_IF_NOT_PRESENT);
                var throwIfNotPresent = Boolean.parseBoolean(
                        throwParam == null
